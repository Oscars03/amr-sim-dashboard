...
### Fixed
- **Every Launch from a `.deb` install failed** with `bash: /opt/IRiSH: No such file or directory`. `launchRos()` builds a `bash -c` string and interpolated four paths into it unquoted; the package installs under `/opt/IRiSH AMR Simulator/` and bash split that on the space. The AppImage mounts at `/tmp/.mount_irish-XXXXXX`, which has no spaces, so only package installs were affected — and the smoke test sources the workspace itself, so it went green on every distro while this was broken.

### Added
- **The smoke test drives the app's own launch path**: it starts `map-server.cjs` (through `ELECTRON_RUN_AS_NODE`, so no `nodejs` package is needed in the image), POSTs `/switch`, and requires `/odom` to publish. The checks above it all pass through a shell command this script writes; this one uses the command the app writes.
...
### Changed
- **Smoke test is no longer at the mercy of the machine it runs on**: DDS discovery is pinned to localhost (a stale peer from another session made every topic check time out while the sim published fine), every `ros2` query uses `--no-daemon` (the daemon caches a graph that outlives the run it came from, so the readiness gate could pass on topics from a sim already killed), and `READY_TIMEOUT` now bounds wall clock rather than counting attempts.
